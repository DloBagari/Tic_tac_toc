...
void ledsWon(byte winner) {
  if (winner){
    PORTB = PORTC = B00000000; //set all pins in PORTB and PORTC  low
    //if  winner is player 1 display aGreen LEDs, otherwire Display Red LEDS
    (winner == 1)? (PORTB |= B101010 ): (PORTB |= B010101); 
    delay(100);
     PORTB = PORTC = B00000000; //set all pins in PORTB and PORTC  low
  }
}
